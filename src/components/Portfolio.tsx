const projects = [
  ...
  },
  {
    ...
    gradient: "from-stone-100/80 via-stone-50/60 to-slate-50/40",
  ...
  },
  {
    ...
    gradient: "from-rose-100/60 via-pink-50/40 to-orange-50/30",
          ...
          {projects.map((project) => (
            <div key={project.title} className="group cursor-pointer">
              <div className={`rounded-2xl h-64 flex items-center justify-center mb-6 overflow-hidden relative bg-gradient-to-br ${project.gradient} glass-card`}>
                ...
                <svg className="relative w-16 h-16 text-[#1c1917]/10 group-hover:text-[#1c1917]/20 group-hover:scale-110 transition-all duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 className="text-xl font-semibold text-[#1c1917] mb-2 group-hover:text-amber-700 transition-colors duration-300">
                {project.title}
              </h3>
              <p className="text-[#1c1917]/50 text-sm">
                {project.description}
              </p>
            </div>
          ))}
