...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-18">
            ...
            <nav className="hidden md:flex items-center gap-9">
              {["Servicios", "Portfolio", "Precios"].map((item) => (
                <Link key={item} href={`/#${item.toLowerCase()}`} className="text-sm text-neutral-600 hover:text-neutral-900 transition-colors">
                  {item}
                </Link>
              ))}
              <Link href="/#contacto" className="btn-primary px-5 py-2.5 rounded-full text-sm font-medium">
      ...
      {/* Mobile overlay menu */}
      <div inert={!menuOpen} className={`fixed inset-0 z-40 bg-[#faf9f7] md:hidden transition-all duration-300 ${menuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="pt-24 px-6 flex flex-col gap-2">
          {["Servicios", "Portfolio", "Precios"].map((item, i) => (
            <Link
              key={item}
          ...
          <div className="pt-6">
            <Link href="/#contacto" onClick={() => setMenuOpen(false)} className="btn-primary block text-center px-6 py-4 rounded-2xl text-sm font-semibold">
              Solicitar cotización
            </Link>
