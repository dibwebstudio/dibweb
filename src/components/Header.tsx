...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0f172a]/95 backdrop-blur border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <button className="md:hidden p-2 text-slate-400" onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu">
        ...
        {menuOpen && (
          <div className="md:hidden pb-4 space-y-2 border-t border-slate-800 pt-4">
            {["Servicios", "Portfolio", "Precios", "Contacto"].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="block px-3 py-2 text-sm text-slate-400 hover:text-white">{item}</a>
            ))}
