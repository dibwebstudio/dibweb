  ...
  return (
    <section id="servicios" className="py-28 px-4 sm:px-6 lg:px-8 bg-[#faf9f7]">
      <div className="max-w-7xl mx-auto">
        ...
        <Reveal delay={100}><div className="border-t border-[#e7e5e0]">
          {services.map((s, i) => (
            <a
              key={s.title}
              href={s.link}
              className="group grid grid-cols-[auto_1fr_auto] sm:grid-cols-[64px_1.4fr_2fr_auto] gap-x-6 gap-y-1 items-baseline py-7 border-b border-[#e7e5e0] hover:bg-white transition-colors px-2 -mx-2 rounded"
            >
              <span className="text-base text-neutral-300 tabular-nums font-medium">{String(i + 1).padStart(2, "0")}</span>
              <h3 className="text-xl sm:text-2xl font-semibold text-neutral-900 group-hover:text-blue-800 [font-family:var(--font-fraunces)] transition-colors">{s.title}</h3>
